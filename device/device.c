...
#include "TimesNewRoman/include/TimesNewRoman/TimesNewRoman.h"
#include "ili9341/include/ili9341/ili9341.h"
#include <string.h>
...
#warning spi/bme280_spi example requires a board with SPI pins
    ...
    // write_register(0xF5, 0x000); // sensor mode 7, 6, 5 bit
    // write_register(0xF4, 101); // oversampling settings x16 pressure 4, 3, 2
    // write_register(0xF4, 010); // oversampling settings x2 temp 7, 6, 5
    // write_register(0xF4, 001); // oversampling settings x1 humidity 0
    // write_register(0xF5, 0x100); // IIR filter settings 0bit
    ...
    while (1) {
        ...
        if (true) {
            ...
            printf("humidity %.2f, ", humidity/1024);///100000);
            ...
            char pres[4];
            char temp[4];
            itoa(humidity/1024, hum, 10);
            itoa(pressure/100, pres, 10);
            itoa(temperature/100, temp, 10);

            clear_buffer();
            enter_a_text(hum, 30, 50, get_buffer());
            enter_a_text(pres, 30, 70, get_buffer());
            enter_a_text(temp, 30, 90, get_buffer());
            display_buffer();
        }
        // putchar(s);
        ...
        // while (true) {
        //     clear_buffer();
        //     enter_a_text(char(humidity), 150, 150, get_buffer());
        //     display_buffer();
